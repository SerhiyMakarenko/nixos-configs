...

{
  ...
  shellInit = ''
    ...
    if [ -z "$__NIX_DARWIN_SET_ENVIRONMENT_DONE" ]
      fenv source /nix/store/zk7pr9glz0fw5xfvnmlxsgdk0adq8x0j-set-environment
    end
  ...
  ssh-add ~/.ssh/rsa/SM.Gitlab.rsa > /dev/null 2>&1";
    ...
    echo -----------------------------------------------------------------
    echo Loaded key for Gitlab: (set_color green)(ssh-add -l | awk '{gsub(/\\/Users\\/serhiy_makarenko/,\"~\")};{print $3}')(set_color normal)
    echo Key type for Gitlab: (set_color green)(ssh-add -l | awk '{print $4}')(set_color normal)
    echo -----------------------------------------------------------------
    echo Hostname is (set_color green)(hostname)(set_color normal)
    echo This Macintosh running (set_color green)(sw_vers -productName) (sw_vers -productVersion)(set_color normal)
    echo You logged in as (set_color green)(whoami)(set_color normal)
    echo Your AirPort IP is (set_color green)(ipconfig getifaddr en0)(set_color normal)
    echo -----------------------------------------------------------------
    uptime
    echo -----------------------------------------------------------------
    who -aH
